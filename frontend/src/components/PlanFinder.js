...
export default function PlanFinder({ user: userProp }) {
  ...
  const user = useMemo(() => location.state?.user ?? userProp ?? null, [location.state, userProp]);
  const postcode = useMemo(() => derivePostcode(searchParams, location, user), [searchParams, location, user]);
  const initialUsage = location.state?.usageHistory || null;

  const { plans, usageHistory, loading, error, fetchPlans, selectedId, select, selectedPlan } =
    usePlans(initialUsage);

  useEffect(() => { if (postcode) fetchPlans(postcode); }, [postcode, fetchPlans]);

  const onConfirm = () =>
    selectedPlan && navigate("/thank-you", { state: { user, postcode, usageHistory, plan: selectedPlan } });
    ...
    <div className="plan-finder">
      ...
      {usageHistory?.length > 0 && (
        <div className="usage-history">
          <h3>Usage History</h3>
          <button onClick={() => setShowUsage(true)}>View Graph</button>
          <ul>
            {usageHistory.map((e, i) => <li key={i}>{e.month} – {e.usage} kWh</li>)}
          </ul>
        </div>
      )}
      ...
      <UsageModal open={showUsage} onClose={() => setShowUsage(false)} usage={usageHistory || []} />
    </div>
  );
}
