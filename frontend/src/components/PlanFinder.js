...
function PlanFinder({ user }) {
  ...
  return (
    <div>
      ...
      <label>Usage: <input type="number" value={monthlyUsage}
        onChange={e => setMonthlyUsage(e.target.value)} /></label>
      <label>Export: <input type="number" value={solarExport}
        onChange={e => setSolarExport(e.target.value)} /></label>
          ...
          return (
            <li key={plan.id}>
              ...
              {plan.solar_feed_in_cents > 0 &&
                <>Feed-in: {plan.solar_feed_in_cents}¢/kWh<br /></>}
              Yearly Cost: ${cost}{monthlyUsage && <> — Savings: ${savings}</>}<br />
              <a href={plan.fact_sheet_url} target="_blank">Fact Sheet</a>
              <hr />
            </li>
          );
        })}
      ...
      {selectedPlanId && (
        <button onClick={() => navigate("/thank-you")}>Confirm Plan</button>
      )}
    </div>
  );
}
